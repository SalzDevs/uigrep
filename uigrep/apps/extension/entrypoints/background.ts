import type { BackgroundRequest, ContentRequest, DaemonStatus } from '../lib/messages';
...
async function token(): Promise<string | undefined> {
  ...
  return typeof stored.pairingToken === 'string' ? stored.pairingToken : undefined;
}

async function activateCurrentTab(): Promise<void> {
  const [tab] = await browser.tabs.query({ active: true, lastFocusedWindow: true });
  if (!tab?.id) return;
  await browser.tabs.sendMessage(tab.id, { type: 'start-capture' } satisfies ContentRequest).catch(() => undefined);
}

async function connect(): Promise<void> {
  const pairingToken = await token();
  if (!pairingToken || socket?.readyState === WebSocket.OPEN || socket?.readyState === WebSocket.CONNECTING) return;
  socket = new WebSocket(`${WS_ORIGIN}/v1/events?token=${encodeURIComponent(pairingToken)}`);
...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener((message: BackgroundRequest, _sender, sendResponse) => {
    ...
    if (message.type === 'capture-visible-tab') {
      void browser.tabs.captureVisibleTab({ format: 'png' }).then(sendResponse);
      return true;
    }

    if (message.type === 'send-capture') {
      void token().then(async (pairingToken) => {
        if (!pairingToken) throw new Error('Open the uigrep extension settings and pair it first.');
        ...
        if (!response.ok) throw new Error(`uigrep daemon returned ${response.status}.`);
