import { captureManifestSchema, captureSessionSchema, type CaptureManifest, type CaptureSession } from '@uigrep/schema';
...
export class UigrepDaemonClient {
  ...
  public async createCapture(capture: CaptureSession): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Unable to store capture: ${response.status}`);
  }

  public async listCaptures(): Promise<CaptureManifest[]> {
    const response = await this.request('/v1/captures');
    if (!response.ok) throw new Error(`Unable to list captures: ${response.status}`);
